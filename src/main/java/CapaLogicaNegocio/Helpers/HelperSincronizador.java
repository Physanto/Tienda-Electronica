...
public class HelperSincronizador {
        ...
        try {
            ...
            for (Sincronizadora sincronizadora : datosPendientes) {
                if (aplicarEnNube(sincronizadora)) {
                    System.out.println("Registro con id: " + sincronizadora.getIdRegistroAfectado()
                            + " fue subido a la nube correctamente");
                    ...
                    System.out.println("No se pudo subir a la nube el registro con id: "
                            + sincronizadora.getIdRegistroAfectado() + ", queda pendiente para el proximo intento");
                }
            }
        } catch (Exception e) {
    ...
    /**
     * Metodo que se encarga de sincronizar la cola de la nube hacia la base de
     * datos local. Se invoca cuando la base
     * local vuelve a estar disponible, para aplicarle todos los cambios (INSERT,
     * UPDATE, DELETE) que se hicieron
     * directamente contra la nube mientras la local estuvo caida. Cada pendiente
     * aplicado se marca como sincronizado
    ...
    public static void SincronizarNubeALocal() {
        ...
        try {
            ArrayList<Sincronizadora> datosPendientes = new SincronizadoraOnlineCRUD()
                    .obtenersNube(Sincronizadora.class);
            ...
            for (Sincronizadora sincronizadora : datosPendientes) {
                ...
                if (!"0".equals(sincronizadora.getEstado()))
                    continue;

                if (aplicarEnLocal(sincronizadora)) {
                    System.out.println("Registro con id: " + sincronizadora.getIdRegistroAfectado()
                            + " fue bajado a local correctamente");
    ...
    /**
     * Aplica en la base de datos local el evento de sincronizacion proveniente de
     * la nube. Reconstruye el objeto a partir
     * del JSON segun la tabla afectada y delega en el DAO local correspondiente.
     * 
     * @param evento el registro de la cola de la nube que se quiere aplicar en
     *               local
     * @return true si la operacion se aplico correctamente en local, de lo
     *         contrario false
     */
        ...
        try {
            switch (tabla) {
                case "Cliente": {
                    ...
                    if (accion == Sincronizadora.Accion.DELETE)
    ...
    /**
     * Aplica en la base de datos de la nube el evento de sincronizacion proveniente
     * de la cola local. Reconstruye el objeto
     * <<<<<<< HEAD
     * tipado a partir del JSON segun la tabla afectada y delega en el OnlineCRUD
     * correspondiente, de modo que las fechas
     * viajan como Date nativo (Firestore Timestamp), igual que en la escritura
     * directa a la nube. Asi ambas bases reciben
     * el mismo formato y no quedan fechas como String.
     * =======
     * tipado a partir del JSON segun la tabla afectada y delega en el CRUD de nube
     * correspondiente. Asi los registros suben
     * con los MISMOS tipos que una escritura directa (Date -> Timestamp, Long como
     * entero, etc.) y se evita que la
     * serializacion generica a Map convierta las fechas en texto y los numeros en
     * double, lo que dejaba la nube inconsistente.
     * >>>>>>> 30223dd (feature: agregando todo el modulo de compras para el cliente
     * y haciendo cambios en clases que fallan...)
     * 
     * @param evento el registro de la cola local que se quiere aplicar en la nube
     * @return true si la operacion se aplico correctamente en la nube, de lo
     *         contrario false
     */
    private static boolean aplicarEnNube(Sincronizadora evento) {
        ...
        try {
            ...
            System.out
                    .println("Error aplicando el evento en la nube (" + tabla + "/" + accion + "): " + e.getMessage());
            return false;
        }
    }

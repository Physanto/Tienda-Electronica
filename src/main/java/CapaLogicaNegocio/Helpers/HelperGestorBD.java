...
public class HelperGestorBD {
    ...
    /**
     * Metodo que se encarga de guardar el registro especificado en la base de datos
     * disponible (local, nube o ambas).
     * Si solo se pudo guardar en una de las dos, deja el pendiente encolado en la
     * cola de sincronizacion correspondiente.
     * 
    ...
    public static <T> boolean guardarRegistro(T object, String coleccion, String id,
        ...
        return ejecutarEscritura(object, coleccion, id, Sincronizadora.Accion.INSERT, metodoGuardarLocal,
                metodoGuardarNube);
    }

    /**
     * Metodo que se encarga de actualizar el registro especificado en la base de
     * datos disponible (local, nube o ambas).
     * Si solo se pudo actualizar en una de las dos, deja el pendiente encolado en
     * la cola de sincronizacion correspondiente.
     * 
    ...
    public static <T> boolean actualizarRegistro(T object, String coleccion, String id,
        ...
        return ejecutarEscritura(object, coleccion, id, Sincronizadora.Accion.UPDATE, metodoActualizarLocal,
                metodoActualizarNube);
    }

    /**
     * Metodo que se encarga de eliminar el registro especificado por id de la base
     * de datos disponible (local, nube o ambas).
     * Si solo se pudo eliminar en una de las dos, deja el pendiente encolado en la
     * cola de sincronizacion correspondiente.
     * 
    ...
    public static boolean eliminarRegistro(String id, String coleccion, Supplier<Boolean> metodoEliminarLocal,
        ...
        return ejecutarEscritura(null, coleccion, id, Sincronizadora.Accion.DELETE, metodoEliminarLocal,
                metodoEliminarNube);
    }

    /**
     * Nucleo bidireccional para las operaciones de escritura (INSERT, UPDATE,
     * DELETE). Intenta aplicar la operacion en
     * cada base de datos que este disponible y delega en
     * {@link #reconciliarEscritura} para decidir si hay que encolar
     * algun pendiente.
     */
    ...
    /**
     * Decide, segun en cuales bases de datos se aplico la operacion, si hay que
     * dejar un pendiente en alguna cola de
     * sincronizacion o respaldar en redundancia cuando ninguna base estuvo
     * disponible.
     */
    ...
    /**
     * Encola el pendiente en la cola LOCAL (tabla ColaSincronizadora) para subirlo
     * a la nube cuando regrese la conexion.
     */
    private static <T> void encolarLocal(T object, String coleccion, String id, Sincronizadora.Accion accion) {
        try {
            ...
            new SincronizadoraDAO().agregar(
                    new Sincronizadora(UUID.randomUUID().toString(), accion, coleccion, id, registroJson, "0"));
    ...
    /**
     * Encola el pendiente en la cola de la NUBE (coleccion Sincronizadora) para
     * bajarlo a local cuando regrese la base local.
     */
    private static <T> void encolarNube(T object, String coleccion, String id, Sincronizadora.Accion accion) {
        try {
            String registroJson = (accion == Sincronizadora.Accion.DELETE) ? "{}" : new Gson().toJson(object);
            new SincronizadoraOnlineCRUD().registrarNube(
                    new Sincronizadora(UUID.randomUUID().toString(), accion, coleccion, id, registroJson, "0"));
    ...
    /**
     * Metodo que se encarga de cargar todos los registros de la base de datos
     * disponible en ese momento. Prioriza la
     * base local; si esta no esta disponible o no devuelve resultados y hay nube,
     * consulta la nube.
     * 
     * @param metodoCargarLocal es el metodo para cargar todos los registros de la
     *                          base de datos local
     * @param metodoCargarNube  es el metodo para cargar todos los registros de la
     *                          base de datos de la nube
     * @example Ejemplo de como usar el metodo
     * 
     *          <pre>{@code
    ...
    /**
     * Metodo que se encarga de cargar el registro con id especificado de la base de
     * datos disponible en ese momento.
     * Prioriza la base local; si esta no esta disponible o no encuentra el registro
     * y hay nube, consulta la nube.
     * 
    ...
    /**
     * Metodo encargado de guardar el registro que no se pudo aplicar en ninguna de
     * las dos bases de datos
     * 
     * @param object es el registro que no se pudo sincronizar en ninguna base de
     *               datos
     */
    ...
    /**
     * Verifica si la base de datos local esta disponible. Abre una conexion de
     * sondeo y la cierra de inmediato para no
     * dejar conexiones colgadas.
     * 
     * @return true si se pudo establecer conexion con la base local, de lo
     *         contrario false
     */

...
public class ClienteControlador {
    ...
    /**
     * Este metodo se encarga de buscar un cliente especifico en la base de datos (local o nube) mediante su ID.
     * @param id El identificador unico del cliente a buscar (capturado desde la vista).
     * @return un record RespuestaControlador con su campo .exito() en true si lo encontro, un mensaje descriptivo
     * y en su campo .datos() la instancia del Cliente encontrado (o null si falla/no existe).
     * @example Ejemplo de uso
     * <pre>{@code
    ...
    /**
     * Este metodo se encarga de validar y actualizar la informacion de un cliente existente,
     * impactando tanto la base de datos local como la nube.
     * @param clienteDTO Objeto de transferencia de datos con la nueva informacion capturada desde la vista.
    ...
    /**
     * Este metodo se encarga de eliminar permanentemente un registro de cliente especifico,
    ...
    /**
     * Este metodo solicita y extrae la lista completa de todos los clientes registrados en el sistema.
     * @return un record RespuestaControlador que, de ser exitoso, contiene un ArrayList de objetos Cliente en su propiedad .datos().
     * @example Ejemplo de uso
     *
     * <pre>{@code
    ...
    private RespuestaControlador<Cliente> validarCamposCliente(ClienteDTO clienteDTO) {
        ...
        if(clienteDTO.id().isEmpty()) return new RespuestaControlador<>(false, "el id no puede estar vacio", null);
        if(HelperValidacion.validarUUID(clienteDTO.id()) > 0) return new RespuestaControlador<>(false, "el id no del tipo esperado (UUID)", null);

        if (clienteDTO.nombre().isEmpty()) return new RespuestaControlador<>(false, "El nombre está vacío", null);
        ...
        if (clienteDTO.apellido().isEmpty()) return new RespuestaControlador<>(false, "El apellido está vacío", null);
        ...
        if (clienteDTO.cedula().isEmpty()) return new RespuestaControlador<>(false, "La cédula es obligatoria", null);
        ...
        if (clienteDTO.direccion().isEmpty()) return new RespuestaControlador<>(false, "La dirección es obligatoria", null);

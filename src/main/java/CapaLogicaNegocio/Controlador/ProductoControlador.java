 ...
 * CADA COSA del negocio debe pasar por el controlador, la vista solo conoce el controlador. nada mas.
 *
...
public class ProductoControlador {
     ...
     * @param productoDTO es un objeto generico para la transferencia de datos al modelo
    ...
    public RespuestaControlador<Producto> agregarProducto(ProductoDTO productoDTO){
        ...
        // por simplicidad y tiempo se va manejar esto en un solo try-catch y un solo mensaje informativo
    ...
    /**
     * Este metodo se encarga de buscar un producto especifico en la base de datos (local o nube) mediante su ID.
     * @param id El identificador unico del producto a buscar (capturado desde la vista).
     * @return un record RespuestaControlador con su campo .exito() en true si lo encontro, un mensaje descriptivo
     * y en su campo .datos() la instancia del Producto encontrado (o null si falla/no existe).
     * @example Ejemplo de uso
     * <pre>{@code
    ...
    /**
     * Este metodo se encarga de validar y actualizar la informacion de un producto existente,
     * impactando tanto la base de datos local como la nube.
     * @param productoDTO Objeto de transferencia de datos con la nueva informacion capturada desde la vista.
    ...
    // los metodos de guardar y actualizar se pueden refactorizar para que quede uno solo ya que
    // hay mucho codigo redundante
    public RespuestaControlador<Producto> actualizarProducto(ProductoDTO productoDTO){

        if(productoDTO == null) return new RespuestaControlador<>(false, "El objeto es nulo", null);
    ...
    /**
     * Este metodo se encarga de eliminar permanentemente un registro de producto especifico,
    ...
    /**
     * Este metodo solicita y extrae la lista completa de todos los productos registrados en el sistema.
     * @return un record RespuestaControlador que, de ser exitoso, contiene un ArrayList de objetos Producto en su propiedad .datos().
     * @example Ejemplo de uso
     *
     * <pre>{@code
     ...
     * Centraliza las validaciones de negocio, nulos y vacios requeridas antes de procesar el ProductoDTO.
     * La vista no tiene acceso a este metodo.
     * @param productoDTO El objeto con los datos crudos procedentes de la capa de presentacion.
    ...
    private RespuestaControlador<Producto> validarCampos(ProductoDTO productoDTO){
        ...
        if(HelperValidacion.validarTodoNombres(productoDTO.marca()) > 0) return new RespuestaControlador<>(false, "marca con formato incorrecto", null);
